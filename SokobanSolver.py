...
class SokobanPuzzle(Search.Problem):
    def __init__(self, warehouse: Warehouse, taboocells = None):
    ...
    def is_move_legal(self, warehouse: Warehouse, move: str) -> bool:
        ...
        if is_coordinate_wall(warehouse, attemptCoordinate):
            ...
            if is_coordinate_wall(warehouse, move_toward_coordinate):
                ...
                return False
            elif move_toward_coordinate in self.taboocells:
                return False
    ...
    def result(self, state: tuple, action: str) -> tuple:
        ...
        attemptCoordinate = move_towards(wh.worker, deltaDir)

        if is_coordinate_box(wh, attemptCoordinate):
            wh.boxes = list(wh.boxes)
            for index, boxCor in enumerate(wh.boxes):
                if boxCor == attemptCoordinate:
                    wh.boxes.pop(index)
                    wh.boxes.insert(index, move_towards(attemptCoordinate, deltaDir))
                    break

        wh.worker = attemptCoordinate
        return self.warehouse_to_state(wh)
    
    def path_cost(self, c, first_state, action, second_state):
        wh = self.state_to_warehouse(state=first_state)
        ...
        if is_coordinate_box(wh, attemptCoordinate):
            for index, boxCor in enumerate(wh.boxes):
                if boxCor == attemptCoordinate:
                    box_weight = self.warehouse.weights[index]
                    break
    ...
    def h(self, node: Search.Node):
        ...
        for index, boxCor in enumerate(node.state[1]):
            ...
            for targetCor in self.warehouse.targets:
                box_distance = find_manhattan(boxCor, targetCor) * (self.warehouse.weights[index] + move_cost)
